...
const DropdownNotification = () => {
  ...
  // Fetch current user ID and user name
  useEffect(() => {
  ...
  // Listen to notifications in real-time and sort them by createdAt (recent first)
  useEffect(() => {
    if (!currentUser) return; // Wait for the currentUser to be set

    const notificationsRef = collection(db, 'notifications');
    const q = query(notificationsRef, where('recipient', '==', currentUser.name)); // Use the name from currentUser

    const unsubscribe = onSnapshot(q, (snapshot) => {
      // Set notifying true only for new notifications
      ...
      newNotifications.sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
  ...
  // Handle clear notifications (both in state and Firestore)
  const handleClearNotifications = async () => {
    try {
      // Clear notifications from the state
      setNotifications([]);

      // Query Firestore to get all notifications for the current user
      const notificationsRef = collection(db, 'notifications');
      const q = query(notificationsRef, where('recipient', '==', currentUser.name)); // Use the name from currentUser
      ...
      snapshot.forEach(async (doc) => {
        await deleteDoc(doc.ref); // Delete the notification from Firestore
      });
          ...
          <div className="absolute -right-27 mt-2.5 flex h-90 w-75 flex-col rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark sm:right-0 sm:w-80">
            ...
            <ul className="flex h-auto flex-col overflow-y-auto">
              {notifications.length === 0 ? (
                <li className="px-4.5 py-3 text-sm text-gray-500">No notifications yet</li>
              ) : (
